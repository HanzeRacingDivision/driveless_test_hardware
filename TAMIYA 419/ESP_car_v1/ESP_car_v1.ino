...
void logSpeed(float targetSpeedToLog, float mesSpeedToLog) {
  ...
  //encoLogFIFO[speedLogItt][0] = encoCount;  encoLogFIFO[speedLogItt][1] = millis();
  speedLogItt++;
...
#define servoPin 27  //servo PWM pin
...
#define motorPin 26  //motor PWM pin
...
void sendSerialFeedback(uint32_t encoVal, int16_t servoAngle, uint32_t timestamp) {
  ...
  /*
//  theSerial.write(encoVal); theSerial.write(encoVal>>8); theSerial.write(encoVal>>16); theSerial.write(encoVal>>24);
...
//  theSerial.write(timestamp); theSerial.write(timestamp>>8); theSerial.write(timestamp>>16); theSerial.write(timestamp>>24);
  ...
  for(uint8_t i=0; i<sizeof(encoVal); i++,j++) { theSerial.write(encoVal>>(i*8)); }
  ...
  */
  ...
  /* // forloop approach (slower(?))
  uint8_t j=0;
  for(uint8_t i=0; i<sizeof(encoVal); i++,j++) { writeBuf[j] = encoVal>>(i*8); }
  ...
  */
  theSerial.write(writeBuf, sizeof(encoVal)+sizeof(servoAngle)+sizeof(timestamp)); // math should be precompiled
  ...
  //    ledcWrite(servoPWMChan, servoTarget); //might as well set this right away (instead of waiting for the next motor control loop
    ...
    if(!serialControlOverride) {
      ...
      if(newRisingTime != lastRisingTime) {
        targetSpeed = throtPWM.normalized() * serialInputSpeedMax;
        servoTarget = degreesToServoPWM(steerPWM.normalized()*serialInputAngleMax); //convert from degrees to servo PWM pulselength
      } else {
...
//    }
    ...
    //Serial.print(encoCount); Serial.print(' '); Serial.print(intifiedSteeringDegrees); Serial.print(' '); Serial.println(millis());
